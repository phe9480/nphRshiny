...
#' p = 0.15; alpha = log(2)/12
#' x = dmcr(t=t, p=0.15, alpha = alpha, tau=6, beta=1, gamma=1, lambda=0.2)
#' 
#' @export
#' 
dmcr = function(t, p=0.3, alpha = log(2)/12, beta=1, gamma=1, lambda=0, tau=0, psi=1) {
  
  S0 = function(t){1-(1-exp(-alpha*t^gamma*exp(lambda*t)))^beta}
  S  = function(t){p + (1-p)*S0(t)}
  
  f0 = function(t){
    A = alpha*beta*t^{gamma-1}*(gamma+lambda*t)*exp(lambda*t-alpha*t^gamma*exp(lambda*t))
    B = (1 - exp(-alpha*t^gamma*exp(lambda*t)))^(1-beta)
    return(A/B)
  }
  
  f = (1-p)*f0(t); f2 = S(tau)^(1-psi)*psi*S(t)^(psi-1)*f
